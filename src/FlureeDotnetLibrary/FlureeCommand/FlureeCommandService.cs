...
{
    public Task<bool> TryCreateCollection(string networkId, string ledgerName, string collectionName,
        string collectionDescription, string verion = "1");

    public Task<string> CreateCollection(string networkId, string ledgerName, string collectionName,
        string collectionDescription, string verion = "1");

    public Task<bool> TryCreatePredicate(string networkId, string ledgerName, string collectionName,
        string predicateName, string predicateDescription, string datatype = "string");

    public Task<string> CreatePredicate(string networkId, string ledgerName, string collectionName,
        string predicateName, string predicateDescription, string datatype = "string");

    public Task<dynamic> Insert(string networkId, string ledgerName,
        List<FlureeTransactionDataParentBody> transactionCommands);
}
...
public class FlureeCommandService : BaseService, IFlureeCommandService
{
    ...
    /// <summary>
    ///     Tries to adds a collection(in relational ledger terms a table) to a given ledger
    ...
    public async Task<bool> TryCreateCollection(string networkName, string ledgerName, string collectionName,
        string collectionDescription, string verion = "1")
    {
        try
        {
    ...
    /// <summary>
    /// Adds a collection(in relational ledger terms a table) to a given ledger
    /// </summary>
    /// <param name="networkName">The network that contains the ledger you want to add to</param>
    /// <param name="ledgerName">The ledger you want to add the collection to</param>
    /// <param name="collectionName">The name of the collection to add</param>
    ...
    /// <returns>
    ///     Returns a string of the command that haapened. Example output.
    ...
    ///     "collection5",-11,true,null],[17592186044440,41,"A test collection to add to
    ///     FLuree",-11,true,null],[17592186044440,42,"1",-11,true,null],
    ...
    ///     [-11,103,1636128750637,-11,true,null],[-11,106,"{\"type\":\"tx\",\"db\":\"reporting/yeetly\",\"tx\":[{\"_id\":\"_collection\",\"name\":\"collection5\",\"doc\":\"A
    ///     test collection to add to FLuree\",\"version\":\"1\"}],\"nonce\":1636128750637,
    ...
    public async Task<string> CreateCollection(string networkName, string ledgerName, string collectionName,
        string collectionDescription, string verion = "1")
    {
        return await FlurlClient.Request($"/fdb/{networkName}/{ledgerName}/transact").PostJsonAsync(
            new List<FlureeCommandModel.FlureeCollectionBody>
            {
                new()
                {
    ...
    /// <summary>
    ///     Adds a predicate(in relational ledger terms a column) to a given predicate
    ...
    /// <param name="datatype">
    ///     The type you want the predicate to be like integer, string etc . Allowed
    ///     types are here https://docs.flur.ee/docs/schema/predicates
    /// </param>
    public async Task<bool> TryCreatePredicate(string networkName, string ledgerName, string collectionName,
        string predicateName, string predicateDescription, string datatype = "string")
    {
        try
        {
            await CreatePredicate(networkName, ledgerName, collectionName, predicateName, predicateDescription,
                datatype);
            return true;
        }
        catch (FlurlHttpException ex)
    ...
    /// <summary>
    /// Adds a predicate(in relational ledger terms a column) to a given collection
    /// </summary>
    /// <param name="networkName">The network that contains the ledger you want to add to</param>
    /// <param name="ledgerName">The ledger you want to add the predicate to</param>
    /// <param name="collectionName">The collection name you want to add the predicate to</param>
    ...
    ///     "collection1/predicate1",-13,true,null],[1001,11,"A test collection to add to
    ///     FLuree",-13,true,null],[1001,12,52776558133249,-13,true,null],
    ...
    ///     \"doc\":\"A test collection to add to
    ///     FLuree\",\"type\":\"string\"}],\"nonce\":1636130382543,\"auth\":\"TfCSi5KvDYKRdkR9RgSsUhgKBZCM9N85iri\",\"expire\":1636130682563}",-13,true,null],
    ...
    public async Task<string> CreatePredicate(string networkName, string ledgerName, string collectionName,
        string predicateName, string predicateDescription, string datatype = "string")
    {
        return await FlurlClient.Request($"/fdb/{networkName}/{ledgerName}/transact").PostJsonAsync(
            new List<FlureeCommandModel.FlureePredicateBody>
            {
                new()
                {
    ...
    /// <returns>
    ///     Returns a json object that shows how long the operation took, what block it was in, and other informational
    ///     data
    /// </returns>
    public async Task<dynamic> Insert(string networkName, string ledgerName,
        List<FlureeTransactionDataParentBody> transactionCommands)
    {
        return await FlurlClient.Request($"/fdb/{networkName}/{ledgerName}/transact").PostJsonAsync(transactionCommands)
            .ReceiveJson();
    }
}